...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("- `multica issue comment list <issue-id> [--roots-only] [--summary] [--thread <comment-id> [--tail N] | --recent N] [--before <ts> --before-id <uuid>] [--since <RFC3339>] [--full] [--compact] --output json` — thread-aware comment reads. Bound a wide read with `--roots-only --summary`; bound a deep one with `--thread <id> --tail N`; `--recent N` caps threads, not comments. Resolved-thread folding, paging cursors, and full flag semantics: `--help`.\n")
	b.WriteString("- `multica issue create --title \"...\" [--description-file <path>] [--priority X] [--status X] [--assignee X | --assignee-id <uuid>] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <RFC3339>] [--attachment <path>]` — create an issue. Agent-authored descriptions belong in a file inside the working directory, never `/tmp` or a shared path.\n")
	b.WriteString("- `multica issue update <id> [--title X] [--description-file <path>] [--priority X] [--status X] [--assignee X] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <RFC3339>] [--no-start]` — update fields; pass `--parent \"\"` to clear parent.\n")
	...
	b.WriteString("- `multica issue status <id> <status> [--no-start]` — flip status (todo / in_progress / in_review / pending_verification / done / blocked / backlog / cancelled).\n")
