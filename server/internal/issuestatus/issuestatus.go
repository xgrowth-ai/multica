...
//
// MODEL. There are 8 categories and they map one-to-one onto the 8 built-in
...
// platform treats them identically — and by that definition the 8 built-ins are
// 8 distinct classes: in_progress, in_review, pending_verification and blocked differ on whether they
...
// The 8 canonical status keys. Each is simultaneously a status key and the
...
// Canonical returns the 8 built-in status keys in display order.
...
// IsBuiltIn reports whether key is one of the 8 canonical statuses.
...
// Ensure idempotently seeds a workspace's 8 built-in statuses. Safe to call
...
//
// The 8 built-in keys resolve even when the workspace has no catalog row for
// them. The catalog EXTENDS the built-in statuses; it does not define them, and
// a workspace has always been able to use all 8. Requiring a row would mean a
